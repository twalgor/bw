...
//  static final boolean VERIFY = true;
  ...
  public static final int MAX_UNC = 10;
  ...
  public int initialLowerBound() {
    ...
    if (VERBOSE) {
      log.log("maximalized " + baseCont);
    }

    obs = baseCont.contract(g);
    return lb;
  }
  ...
  Minor improve() {
    ...
    bestContractor[baseCont.nCont] = baseCont;
    ...
    while (true) {
      if (TRACE) {
        ...
        for (Contractor cont: queue) {
          System.out.println(" " + cont);
        }
        System.out.println("bests:");
        for (int k = g.n - BASE_SIZE; k <= baseCont.nCont; k++) {
          Contractor cont = bestContractor[k];
          if (cont != null) {
      ...
      if (!queue.isEmpty()) {
        Contractor cont = queue.remove();
        if (TRACE) {
          System.out.println("dequeueed " + cont);
        }
        if (cont.score == 0 && cont.clears(g)) { 
          solution = cont;
          if (TRACE) {
        ...
        for (int i = g.n - BASE_SIZE; i <= baseCont.nCont; i++) {
          ...
          if (TRACE) {
            System.out.println("trying best at " + i + " " + cont);
          }
...
//      }
      for (int s = 1; s <= Math.min(MAX_UNC, BASE_SIZE - graphSize()); s++) {
        XBitSet uncont = randomSubset(contSet, s);
        Contractor cont = new Contractor(contSet.subtract(uncont));
        cont.score();
//        if (TRACE) {
//          System.out.println("trying cont " + cont);
//        }
        int k = nCont - s;
        if (bestContractor[k] == null ||
            cont.compareTo(bestContractor[k]) < 0) {
//          if (TRACE) {
//            System.out.println("improved cont " + cont);
//          }
      ...
      for (int s = 1; s <= Math.min(MAX_UNC, baseCont.nCont - nCont); s++) {
        XBitSet addCont = randomSubset(baseCont.contSet.subtract(contSet), s);
        Contractor cont = new Contractor(contSet.unionWith(addCont));
        cont.score();
        int k = nCont + s;
//        if (TRACE) {
//          System.out.println("trying cont " + cont);
...
//    test("../instance/PACE2017bonus_gr/", "Promedus_38_14", 10);
  ...
  private static void test(String path, String name, int upperBound) {
    log = new Log("Multifilter" + VERSION + "_" + BASE_SIZE + "_" + MAX_UNC, 
        name);
    ...
    MultiFilter1 mf = new MultiFilter1(local.h);
