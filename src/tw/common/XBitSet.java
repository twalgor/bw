...
public class XBitSet extends BitSet 
  ...
  */
  ...
  public int randomSetBit(Random random) {
    int[] a = toArray();
    return a[random.nextInt(a.length)];
  }
  
  public static XBitSet largest(Collection<XBitSet> col) {
    XBitSet largest = null;
    for (XBitSet x: col) {
      if (largest == null || 
          cardinalityComparator.compare(x, largest) > 0) {
        largest = x;
      }
    }
    return largest;
  ...
  public static ArrayList<XBitSet> largests(Collection<XBitSet> col) {
    ArrayList<XBitSet> largests = new ArrayList<>();
    int max = 0;
    for (XBitSet x: col) {
      if (largests.isEmpty() || 
          x.cardinality() == max) {
  ...
  public static XBitSet smallest(Collection<XBitSet> col) {
    XBitSet smallest = null;
    for (XBitSet x: col) {
      if (smallest == null || 
          cardinalityComparator.compare(x, smallest) < 0) {
        smallest = x;
      }
    }
    return smallest;
  ...
  public static Set<XBitSet> convertAll(Set<XBitSet> sets, int[] map) {
    Set<XBitSet> converted = new HashSet<>();
    for (XBitSet vs: sets) {
      converted.add(vs.convert(map));
    }
    return converted;
  }
