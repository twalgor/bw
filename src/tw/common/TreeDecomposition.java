...
//  private static boolean debug = true;
  ...
  /**
   * Make a copy of this tree-decomposition
   *
   * @return the copy created
   */
  
  public TreeDecomposition copy() {
    TreeDecomposition td = new TreeDecomposition(nb, width, g);
    for (int b = 1; b <= nb; b++) {
      td.bags[b] = Arrays.copyOf(bags[b], bags[b].length);
      td.degree[b] = degree[b];
      td.neighbor[b] = Arrays.copyOf(neighbor[b], neighbor[b].length);
    }
    return td;
  }
  ...
  /**
   * Computes a partial perfect elimination order of the chordal graph obtained by 
   * filling all bags of this tree-decomposition into cliques, leaving the specified
   * vertices unordered.
   * @param toLeave the set of vertices to be left unordered; must be a subset of some bag
   * @return the perfectElimination order
   */
  
  public int[] perfectEliminationOrderLeaving(XBitSet toLeave) {
    XBitSet removed = new XBitSet();
    XBitSet ordered = new XBitSet();
    Queue<Integer> queue = new LinkedList<>();
    int r = 0;

    for (int b = 1; b <= nb; b++) {
      if (r == 0 && toLeave.isSubset(new XBitSet(bags[b]))) {
        r = b;
      }
    }
    assert r > 0;
    ...
    for (int b = 1; b <= nb; b++) {
      if (b != r && bags[b].length <= 1) {
        queue.add(b);
      }
    }
    int[] ord = new int[g.n - toLeave.cardinality()];
    int j = 0;
    while (!queue.isEmpty()) {
      int b = queue.remove();
      int v = uniqueVetex(b, removed, ordered);
      if (v == -1) {
        removed.set(b);
        for (int i = 0; i < degree[b]; i++) {
          int b1 = neighbor[b][i];
          if (b1 != r && 
              !removed.get(b1) && remainingDegree(b1, removed) == 1) {
            queue.add(b1);
          }
        }
      }
    ...
    assert removed.cardinality() == nb - 1;
    assert !removed.get(r);
    for (int v: bags[r]) {
      if (!ordered.get(v) && !toLeave.get(v)) {
        ord[j++] = v;
      }
    }
    assert j == ord.length;
  ...
  /**
   * Computes a perfect elimination order of the chordal graph obtained by 
   * filling all bags of this tree-decomposition into cliques;
   * 
   * @return the perfectElimination order
   */
  
  public int[] perfectEliminationOrder() {
    XBitSet removed = new XBitSet();
    XBitSet ordered = new XBitSet();
    Queue<Integer> queue = new LinkedList<>();
    for (int b = 1; b <= nb; b++) {
    ...
    while (!queue.isEmpty()) {
      int b = queue.remove();
//      System.out.println("j " + j + " b " + b + " removed " + removed  + " ordered " + ordered);
      int v = uniqueVetex(b, removed, ordered);
      while (v >= 0) {
//        System.out.println(" j " + j + " v " + v + " ordered " + ordered);
        ord[j++]= v;
        ordered.set(v);
        v = uniqueVetex(b, removed, ordered);
      }
      removed.set(b);
      for (int i = 0; i < degree[b]; i++) {
        int b1 = neighbor[b][i];
        if (!removed.get(b1) && remainingDegree(b1, removed) == 1) {
          queue.add(b1);
        }
      }
    }
  ...
  int remainingDegree(int b, XBitSet removed) {
    int d = 0;
    for (int i = 0; i < degree[b]; i++) {
      int b1 = neighbor[b][i];
      if (!removed.get(b1)) {
  ...
  private int uniqueVetex(int b, XBitSet removed, XBitSet ordered) {
    XBitSet cand = new XBitSet(bags[b]).subtract(ordered); 
    for (int i = 0; i < degree[b]; i++) {
      int b1 = neighbor[b][i];
      if (!removed.get(b1)) {
        cand.andNot(new XBitSet(bags[b1]));
      }
    }
    return cand.nextSetBit(0);
  }
  ...
  /**
   * Saves this tree-decomposition in the file specified by a path string,
   * in .td format with comment lines.
   * A stack trace will be printed if the file is not available for writing
   * @param path the path-string
   * @param comments ArrayList of comment lines
   */
  public void save(String path, ArrayList<String> comments) {
...
//	      
	      if (!s[1].equals("td")) {
	        s = new String[] {s[0], "td", s[1], s[2], s[3]};
	      }
...
//	      System.out.println("nb = " + nb + ", width = " + width + ", n = " + n);
  ...
  public Set<XBitSet> setOfBags() {
    Set<XBitSet> bagSet = new HashSet<>();
    for (int b = 1; b <= nb; b++) {
      bagSet.add(new XBitSet(bags[b]));
    }
    return bagSet;
  }
  
  public Set<XBitSet> setOfSeparators() {
    Set<XBitSet> sepSet = new HashSet<>();
    for (int b = 1; b <= nb; b++) {
      XBitSet bag = bagAt(b);
      for (int b1: neighbor[b]) {
        XBitSet sep = bag.intersectWith(bagAt(b1));
        sepSet.add(sep);
      }
    }
    return sepSet;
  ...
  public Graph toTree() {
    Graph t = new Graph(nb + 1);
    t.neighborSet[0] = new XBitSet();
    for (int b = 1; b <= nb; b++) {
      t.neighborSet[b] = new XBitSet(neighbor[b]);
    }
    return t;
  }
  
  public XBitSet bagAt(int b) {
    return new XBitSet(bags[b]);
  }
  
  public XBitSet unionBags(XBitSet ns) {
    XBitSet union = new XBitSet();
    for (int b = ns.nextSetBit(0); b >= 0; b = ns.nextSetBit(b + 1)) {
      union.or(new XBitSet(bags[b]));
    }
    return union;
  }
}
