...
public class Graph {
  ...
  public int numberOfEdges(XBitSet vs) {
    int count = 0;
    for (int v: vs.toArray()) {
      count += neighborSet[v].intersectWith(vs).cardinality();
    }
    return count / 2;
  }
...
//    }
  ...
  public boolean isWellSeparating(XBitSet bag) {
    return isWellFormedBag(bag);
  }
  
  public boolean isWellFormedBag(XBitSet bag) {
    ArrayList<XBitSet> components = separatedComponents(bag);
    for (XBitSet compo: components) {
      XBitSet sep = neighborSet(compo);
      if (sep.equals(bag)) {
        return false;
      }
      ArrayList<XBitSet> fulls = fullComponents(sep);
      if (fulls.size() <= 1) {
        return false;
      }
      XBitSet inner = null;
  ...
  */
   ...
   * @param path the path-string
   */
  public void saveAdj(String path) {
    File outFile = new File(path);
    PrintStream ps;
    try {
      ps = new PrintStream(new FileOutputStream(outFile));
   ...
   * @param ps print stream
   */
  public void writeAdjTo(PrintStream ps) {
    for (int v = 0; v < n; v++) {
      ps.print(v);
      for (int w: neighborSet[v].toArray()) {
        if (v < w) {
          ps.print(" " + w);
        }
      }
...
//      System.out.println("      " + bijection[v] + ":" + g.neighborSet[bijection[v]]);
  ...
  public Edge[] edgeArray() {
    ArrayList<Edge> list = edgeList();
    Edge[] edges = new Edge[list.size()];
    return list.toArray(edges);
  }

  public void addVertex() {
    neighborSet = Arrays.copyOf(neighborSet, n + 1);
    neighborSet[n] = new XBitSet();
    n++;
  }
  
  public int degree(int v) {
    return neighborSet[v].cardinality();       
  }
  
  public int[] neighborsArray(int v) {
    return neighborSet[v].toArray();
  }
  
  public boolean isCycleCover() {
    for (int v = 0; v < n; v++) {
      if (neighborSet[v].cardinality() != 2) {
        return false;
      }
    }
    return true;
  ...
  public boolean isCycle() {
    return isCycleCover() && isConnected(all);
  }

  public boolean isTree() {
    return isConnected(all) && numberOfEdges() == n - 1;
  }

  public boolean isStar() {
    return isTree() && maxDegree() == n - 1;
  }
  ...
  public int maxDegree() {
    ...
    int max = neighborSet[0].cardinality();
    for (int v = 0; v < n; v++) {
      int d = neighborSet[v].cardinality();
      if (d > max) max = d;
    }
